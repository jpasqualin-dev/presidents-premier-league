(function () {
    ...
    function open(options) {
        if (window.DrawerRouter && !options.skipRouter) {
            window.setSharedMatchDrawerTrigger?.(document.activeElement);
            return window.DrawerRouter.open({
                type: 'weekly',
                id: String(options.week ?? ''),
                render: () => open({ ...options, skipRouter: true })
            });
        }
        ...
        if (!options.skipRouter) window.DrawerRouter?.closeAll();
        render();
    }
    ...
    document.addEventListener('drawer-router-back', event => {
        const matchId = event.detail?.to?.type === 'weekly' && event.detail?.from?.type === 'match' ? event.detail.from.id : '';
        if (!matchId) return;
        const trigger = document.querySelector(`.weekly-form-match[data-match-id="${CSS.escape(matchId)}"]`);
        const owner = trigger?.closest('.weekly-detail-leader');
