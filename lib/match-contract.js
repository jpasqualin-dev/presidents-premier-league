...
const { longTeamName, crestForTeam } = require('./team-names');
...
function normalizeEspnEvent(event) {
    ...
    const displayClock = competition.status?.displayClock || status.displayClock || null;
    ...
    return {
        ...
        homeTeam: { id: String(home.team.id), name: longTeamName(home.team.displayName, home.team.id), crest: crestForTeam(home.team.displayName, home.team.id) || home.team.logo || null },
        awayTeam: { id: String(away.team.id), name: longTeamName(away.team.displayName, away.team.id), crest: crestForTeam(away.team.displayName, away.team.id) || away.team.logo || null },
        score: {
            fullTime: {
...
function normalizeNeonMatch(row) {
    const state = normalizeMatchStatus({
        state: row.status_state || row.status,
    ...
    return {
        ...
        homeTeam: { id: row.home_provider_id, name: longTeamName(row.home_name, row.home_provider_id), crest: crestForTeam(row.home_name, row.home_provider_id) || row.home_logo },
        awayTeam: { id: row.away_provider_id, name: longTeamName(row.away_name, row.away_provider_id), crest: crestForTeam(row.away_name, row.away_provider_id) || row.away_logo },
        score: {
            fullTime: { home: row.home_score, away: row.away_score },
            halfTime: { home: row.home_half_time_score, away: row.away_half_time_score }
        },
        scorers: row.scorers || [],
        events: (row.events || []).filter(event => !isDelayEvent(event) && !event.scoringPlay && !/^goal|score/i.test(String(event.eventType || event.type || ''))).map(event => ({
            ...event,
            substitution: Boolean(event.substitution || String(event.eventType || '').toLowerCase().includes('substitution'))
        })),
